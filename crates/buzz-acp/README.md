...
| `!cancel` | The owner may cancel the current in-flight turn for the channel. Another author admitted by the inbound gate may cancel only an in-flight turn triggered by that author's own event. |
...
Control commands must be kind:9 stream messages, mention this agent with a `p` tag, and are consumed by the harness instead of being forwarded to the agent. `!shutdown` and `!rotate` remain owner-only. `!cancel` is also accepted from an author admitted by the inbound gate, but the author-scoped signal cannot cancel another member's turn; when no matching turn is active it is consumed as a no-op.
