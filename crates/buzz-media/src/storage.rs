        ...
        match &self.backend {
            MediaBackend::S3(bucket) => {
                let identifiers = keys
                    ...
                    .collect::<Vec<_>>();
                self.delete_object_identifiers(bucket, identifiers).await
            }
            MediaBackend::Azure(store) => store.delete_objects(keys).await,
        }
    }

    /// Verify that version-aware listing is safe and reachable.
    ///
    /// S3 is checked through its native version-list API. Azure deployments
    /// currently support unversioned containers only, so the adapter fails
    /// closed when a probe reports Blob versioning and separately proves
    /// prefix-list access.
    pub async fn preflight_version_listing(&self, prefix: &str) -> Result<(), MediaError> {
        match &self.backend {
            MediaBackend::S3(_) => self
                .list_prefix_versions_page(prefix, None, None, 1)
                .await
                .map(|_| ()),
            MediaBackend::Azure(store) => {
                let probe = format!("probe/deletion-versioning-{}", uuid::Uuid::new_v4());
                if store.versioning_detected(&probe).await? {
                    return Err(MediaError::StorageError(
                        "Azure Blob versioning is enabled, but exact-version deletion is not supported"
                            .to_string(),
                    ));
                }
                store.list_prefix_page(prefix, None, 1).await.map(|_| ())
            }
        }
    }

    /// Bulk-delete exact object versions.
    ///
    /// S3 identifiers carry explicit version ids. The Azure adapter accepts
    /// only the empty version ids emitted for a preflight-verified unversioned
    /// container and otherwise fails closed.
    ...
    async fn delete_object_versions_with_folding(
        ...
        match &self.backend {
            ...
            MediaBackend::Azure(store) => {
                if versions
                    .iter()
                    .any(|version| !version.version_id.is_empty())
                {
                    return Err(MediaError::StorageError(
                        "Azure exact-version deletion is not supported".to_string(),
                    ));
                }
                let keys = versions
                    .iter()
                    ...
                    .collect::<Vec<_>>();
                store.delete_objects(&keys).await
            }
        }
    }
    ...
    pub async fn list_prefix_versions_page(
        ...
        match &self.backend {
            ...
            MediaBackend::Azure(store) => {
                ...
                {
                    return Err(MediaError::StorageError(
                        "Azure version-id pagination is not supported".to_string(),
                    ));
                }
                let page = store.list_prefix_page(prefix, key_marker, max_keys).await?;
                let next_key_marker = page.next_continuation_token;
                Ok(ObjectVersionsPage {
                    entries: page
                        .objects
                        .into_iter()
                        .map(|(key, size)| ObjectVersionEntry {
                            key,
                        ...
                        .collect(),
                    next_version_id_marker: page.is_truncated.then(String::new),
                    next_key_marker,
