...
Remove an obsolete announcement with `repos delete --id <id>`. It publishes a verified coordinate tombstone and returns exit code 5 if a newer concurrent repository head survives, so never report deletion until the command succeeds.
