...
Upstream baseline: `block/buzz@63f961c7e4818a1d29f1185002c123e486bd4a19`
...
| Make relay and push-gateway GHCR namespaces fork-configurable | `.github/workflows/docker.yml` in the post-PR #68 fork-publication repair | Current upstream parameterizes the relay image only and still hardcodes the push-gateway package, cache, attestation, and verification owner to `block`; a fork main push therefore cannot publish the otherwise-green gateway build | Protected container builds followed by a main push that publishes both multi-architecture manifests and provenance to the configured repository-owner namespace |
